...
handle_info({ _,undefined,[_,{<<"Ref">>,signedint,Ref},{<<"Dkim">>,longstr,Dkim},{<<"Date">>,longstr,Date},_,_,{<<"DKIM_VALID">>,signedint,Dkim_valid}],_Hop}, #state{addr=AdresseDeMerde}=State) when Ref =:= AdresseDeMerde ->
%       gen_server:cast(_SenderPid, {_AdresseDeMerde,[]}),
	...
	gproc:send({p, l, Ref}, {message2,Date,Dkim,Dkim_valid}),	
        {noreply, State};
handle_info({ _,undefined,[_,{<<"Ref">>,signedint,_Ref},_,_,_,_,_],_Hop}, #state{addr=_AdresseDeMerde}=State) ->
